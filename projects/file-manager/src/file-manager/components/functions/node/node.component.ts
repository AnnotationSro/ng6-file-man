...
export class NodeComponent implements OnInit {
  ...
  private open() {
    ...
    if (this.node.stayOpen) {
      ...
      this.store.dispatch(new ACTIONS.SetPath(this.node.pathToNode));
      return;
    }
  ...
  private showMenu() {
    this.store.dispatch(new ACTIONS.SetSelectedNode(this.node));
  }
  ...
  private setNodeSelectedState() {
    if (!this.node.isExpanded) {
      document
        .getElementById("tree_" + this.node.pathToNode)
        .classList.add("deselected");

      this.nodeService.foldRecursively(this.node);

      this.store.dispatch(new ACTIONS.SetPath(this.node.pathToParent));
    } else {
      document
        .getElementById("tree_" + this.node.pathToNode)
        .classList.remove("deselected");
    }
  }
}
